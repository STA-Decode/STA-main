
//original
package org.firstinspires.ftc.teamcode.STAuton;

import com.qualcomm.hardware.lynx.LynxModule;
import com.qualcomm.robotcore.eventloop.opmode.Autonomous;
import com.qualcomm.robotcore.eventloop.opmode.LinearOpMode;
import com.qualcomm.robotcore.util.ElapsedTime;

import org.firstinspires.ftc.teamcode.RobotParts.Motors;
import org.firstinspires.ftc.teamcode.RobotParts.ServoTest;
import org.firstinspires.ftc.teamcode.RobotParts.DriveTrain;

import java.util.List;

@Autonomous(name = "AutonomousRedClose", group = "AutonomousRedClose")
//Naam van project
public class AutonomousRedClose extends LinearOpMode {
    private ElapsedTime runtime = new ElapsedTime();
    private final int TICKS_PER_REVOLUTION = 8192;
    private final double WHEEL_DIAMETER_MM = 34.0;
    private final double TICKS_PER_CM = TICKS_PER_REVOLUTION / (WHEEL_DIAMETER_MM * Math.PI);

    @Override
    public void runOpMode() throws InterruptedException {
        List<LynxModule> allHubs = hardwareMap.getAll(LynxModule.class);

        for (LynxModule hub : allHubs) {
            hub.setBulkCachingMode(LynxModule.BulkCachingMode.AUTO);
        }


        DriveTrain driveTrain = new DriveTrain();
        Motors motors = new Motors();
        ServoTest feeder = new ServoTest();
        motors.init(hardwareMap);
        driveTrain.init(hardwareMap);
        feeder.init(hardwareMap);

        int state = 0;

        double startuptime;
        double error = 0;
        int ticks = 0; //voor-achter ( met shooter mee)//
        int ticks2 = 0; //link-rechts//
        boolean stateInitialized = false;

        waitForStart();
        if (isStopRequested()) return;

        startuptime = System.currentTimeMillis();

        while (opModeIsActive() && state != 27) {
            switch (state) {
                case 0:
                    error = 26.5 - ticks / TICKS_PER_CM;
                    motors.shootingMethod(1);
                    driveTrain.drive(0.7, Math.PI, 0, true);
                    ticks = driveTrain.getEncoderData()[0];
                    ticks2 = driveTrain.getEncoderData()[1];
                    telemetry.addData("ticks", ticks);
                    telemetry.addData("ticks2", ticks2);
                    if (ticks < -13987 || System.currentTimeMillis() > startuptime + 2400) {
                        startuptime = System.currentTimeMillis();
                        state++;
                        driveTrain.drive(0, 0, 0, true);

                        feeder.setSevenPos(0);
                    }
                    break;
                case 1:
                    if (System.currentTimeMillis() > startuptime + 2500) {
                        state++;
                        feeder.setSevenPos(0.475);
                    }
                    break;
                case 2:
                    if (System.currentTimeMillis() > startuptime + 3000) {
                        state++;
                        feeder.setSevenPos(0);
                    }
                    break;
                case 3:
                    if (System.currentTimeMillis() > startuptime + 3500) {
                        state++;
                        motors.intakeMethod(-1);
                        motors.transferMethod(1);
                    }
                    break;
                case 4:
                    if (System.currentTimeMillis() > startuptime + 3800) {
                        state++;
                        feeder.setSevenPos(0.475);
                        motors.transferMethod(0);
                        motors.intakeMethod(0);
                    }
                    break;
                case 5:
                    if (System.currentTimeMillis() > startuptime + 4300) {
                        state++;
                        feeder.setSevenPos(0);
                    }
                    break;
                case 6:
                    if (System.currentTimeMillis() > startuptime + 4600) {
                        state++;
                        motors.intakeMethod(-1);
                        motors.transferMethod(1);
                    }
                    break;
                case 7:
                    if (System.currentTimeMillis() > startuptime + 5000) {
                        state++;
                        motors.transferMethod(0);
                        motors.intakeMethod(0);
                        feeder.setSevenPos(0.475);
                    }
                    break;
                case 8:
                    if (System.currentTimeMillis() > startuptime + 5500) {
                        state++;
                        feeder.setSevenPos(0);
                    }
                    break;
                case 9:
                    if (System.currentTimeMillis() > startuptime + 6000) {
                        motors.shootingMethod(0);
                        state++;
                    }
                    break;
                case 10:
                    if (System.currentTimeMillis() > startuptime + 6000) {
                        motors.shootingMethod(0);
                        state++;
                    }
                    break;

//                case 10:
//                    if (!stateInitialized) {
//                        driveTrain.resetEncoders();
//                        stateInitialized = true;
//                    }
//                    ticks = driveTrain.getEncoderData()[0];
//                    error = 65.0 - ticks / TICKS_PER_CM;
//                    telemetry.addData("ticks", ticks);
//                    driveTrain.drive(0.7, Math.PI, 0, true);
//                    if (ticks < -21528 || System.currentTimeMillis() > startuptime + 3337000) {
//                        startuptime = System.currentTimeMillis();
//                        state++;
//                        stateInitialized = false;
//                        driveTrain.drive(0, 0, 0, true);
//                    }
//                    break;
//                case 11:
//
//                    if (!stateInitialized) {
//                        driveTrain.resetEncoders();
//                        stateInitialized = true;
//                    }
//                    ticks = driveTrain.getEncoderData()[1];
//                    error = 25.0 - ticks / TICKS_PER_CM;
//                    driveTrain.drive(0, 0, 0.5, true);
//                    if (ticks < -3394 || System.currentTimeMillis() > startuptime + 33311000) {
//                        startuptime = System.currentTimeMillis();
//                        motors.transferMethod(0.5);
//                        motors.intakeMethod(0.5);
//                        state++;
//                        stateInitialized = false;
//
//                        driveTrain.drive(0, 0, 0, true);
//                    }
//                    break;
//                case 12:
//                    if (!stateInitialized) {
//                        driveTrain.resetEncoders();
//                        stateInitialized = true;
//                    }
//                    ticks = driveTrain.getEncoderData()[0];
//                    error = 60.0 - ticks / TICKS_PER_CM;
//                    driveTrain.drive(-0.7, Math.PI, 0, true);
//                    if (ticks < -49316 || System.currentTimeMillis() > startuptime + 33314000) {
//                        startuptime = System.currentTimeMillis();
//                        motors.transferMethod(0.5);
//                        motors.intakeMethod(0.5);
//                        state++;
//                        stateInitialized = false;
//
//                        driveTrain.drive(0, 0, 0, true);
//                    }
//                    break;
//                case 13:
//                    if (System.currentTimeMillis() > startuptime + 15500) {
//                        motors.transferMethod(0);
//                        motors.intakeMethod(0);
//                        state++;
//                    }
//                    break;
//                case 14:
//                    if (!stateInitialized) {
//                        driveTrain.resetEncoders();
//                        stateInitialized = true;
//                    }
//                    ticks = driveTrain.getEncoderData()[1];
//                    error = 25.0 - ticks / TICKS_PER_CM;
//                    driveTrain.drive(0, 0, -0.5, true);
//                    if (ticks < -7983 || System.currentTimeMillis() > startuptime + 33316000) {
//                        startuptime = System.currentTimeMillis();
//                        state++;
//                        stateInitialized = false;
//                        motors.transferMethod(0);
//                        motors.intakeMethod(0);
//                        driveTrain.drive(0, 0, 0, true);
//
//
//                    }
//                    break;
//                case 15:
//                    if (!stateInitialized) {
//                        driveTrain.resetEncoders();
//                        stateInitialized = true;
//                    }
//                    ticks = driveTrain.getEncoderData()[0];
//                    error = 60.0 - ticks / TICKS_PER_CM;
//                    driveTrain.drive(-0.7, Math.PI, 0, true);
//                    if (ticks < -19726 || System.currentTimeMillis() > startuptime + 33319000) {
//                        startuptime = System.currentTimeMillis();
//                        state++;
//                        stateInitialized = false;
//                        driveTrain.drive(0, 0, 0, true);
//                    }
//                    break;
//                case 16:
//                    if (!stateInitialized) {
//                        driveTrain.resetEncoders();
//                        stateInitialized = true;
//                    }
//                    ticks = driveTrain.getEncoderData()[1];
//                    error = 25.0 - ticks / TICKS_PER_CM;
//                    driveTrain.drive(0, 0, 0.5, true);
//                    if (ticks < -2048 || System.currentTimeMillis() > startuptime + 33321000) {
//                        startuptime = System.currentTimeMillis();
//                        state++;
//                        stateInitialized = false;
//                        driveTrain.drive(0, 0, 0, true);
//                    }
//                    break;
//                // tot hier//
//                case 17:
//                    if (System.currentTimeMillis() > startuptime + 22500) {
//                        state++;
//                        feeder.setSevenPos(0.475);
//                    }
//                    break;
//                case 18:
//                    if (System.currentTimeMillis() > startuptime + 23000) {
//                        state++;
//                        feeder.setSevenPos(0);
//                    }
//                    break;
//                case 19:
//                    if (System.currentTimeMillis() > startuptime + 23500) {
//                        state++;
//                        motors.intakeMethod(-1);
//                        motors.transferMethod(1);
//                    }
//                    break;
//                case 20:
//                    if (System.currentTimeMillis() > startuptime + 23800) {
//                        state++;
//                        feeder.setSevenPos(0.475);
//                        motors.transferMethod(0);
//                        motors.intakeMethod(0);
//                    }
//                    break;
//                case 21:
//                    if (System.currentTimeMillis() > startuptime + 24300) {
//                        state++;
//                        feeder.setSevenPos(0);
//                    }
//                    break;
//                case 22:
//                    if (System.currentTimeMillis() > startuptime + 24600) {
//                        state++;
//                        motors.intakeMethod(-1);
//                        motors.transferMethod(1);
//                    }
//                    break;
//                case 23:
//                    if (System.currentTimeMillis() > startuptime + 25000) {
//                        state++;
//                        motors.transferMethod(0);
//                        motors.intakeMethod(0);
//                        feeder.setSevenPos(0.475);
//                    }
//                    break;
//                case 24:
//                    if (System.currentTimeMillis() > startuptime + 25500) {
//                        state++;
//                        feeder.setSevenPos(0);
//                    }
//                    break;
//                case 25:
//                    if (System.currentTimeMillis() > startuptime + 26000) {
//                        motors.shootingMethod(0);
//                        state++;
//                    }
//                    break;
//                case 26:
//                    if (!stateInitialized) {
//                        driveTrain.resetEncoders();
//                        stateInitialized = true;
//                    }
//                    ticks = driveTrain.getEncoderData()[1];
//                    error = 25.0 - ticks / TICKS_PER_CM;
//                    driveTrain.drive(0, 0, -0.5, true);
//                    if (ticks < -975 || System.currentTimeMillis() > startuptime + 33321000) {
//                        startuptime = System.currentTimeMillis();
//                        state++;
//                        stateInitialized = false;
//                        driveTrain.drive(0, 0, 0, true);
//                    }
//                    break;
//            }
        }
    }
}